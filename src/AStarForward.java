...
//h(n)=Manhattan Distance of current node and the goal node
	...
	public AStarForward(Boolean[][] maze, MetaDataBO initialData, int noOfUnblockedCells, int gType)
	{
		this.maze = maze;
	...
	public boolean startRepeatedForwardAStar(){
		if(checkStartEndConditions()){
			...
			if(!agentsPath.isEmpty()){
				System.out.println("No of movements : "+(agentsPath.size() - 1));
			}
			return true;
		}
		else{
	...
	public boolean isNodeToBeGenerated(int x, int y){
		if((!isGoalFound) && (!expandedNodesForTrack.contains(new AStarBO(x, y))) && (!nodesObserved.contains(new AStarBO(x, y)))){
			return true;
		}
		return false;
	}
